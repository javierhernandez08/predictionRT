...
# Configuración de la key de Gemini y del modelo.
load_dotenv()
...
# Metodos de preparación de los datos


# Buscamos columnas en el Excel
...
# Prepara el dataframe que luego se usa para el contexto y selección.
...
# Divide el EXCEL en 80% (contexto) y 20% (predicción)
...
# Construye la tabla de contexto que se le enviará a Gemini a raiz de EXCEL subido.
...
# Lee la respuesta de Gemini, la limpia y une para su visualización final.
...
# Busca en los mensajes el numero de segundos antes de reintento.
...
# Envia el mensaje, lee el texto e intenta las repeticiones hasta un máximo de 10 segundo entre esperas.
...
# Busca el primer numero de la respuesta o devuelve un error.
...
# Generamos la aplicación y permitimos el CORS con la app.
app = Flask(__name__)
CORS(app)


# Definimos la ejecución del home si se viene de la ruta /
@app.route("/")
def home():
    return HTML


# Saca los nombres de las hojas del excel que se sube.
...
# Guardamos el archivo .pdf temporalmente para su envio a Gemini
...
# Funcion que envia los datos a Gemini(Lee excel, sube paper,crea tabla de contexto) y guarda la respuesta.
...
def load():
    ...
    # AHORA: 80% contexto, 20% predicción
    ...
    state.update({
        "chat": chat,
        "df80": rest20,  # AHORA ES EL 20% PARA PREDECIR
        "col_rt": col_rt,
    ...
    return jsonify({
        "ok": True,
        "total20": len(rest20),  # 20% para predecir
        "total80": len(ctx80),   # 80% de contexto
        "col_rt": col_rt,
        "col_smiles": col_smiles,
...
# Funcion que gestiona los escenarios sin EXCEL. Pudiendo ser con paper o sin nada.
...
# Manda el smiles que se introduce a mano, lee la respuesta y trunca la respuesta para quedarse solo con el número.
...
# Manda el excel, genera el prompt y lee la respuesta que recibe de Gemini.
...
# Obtiene los logs que el sistema va guardando.
@app.route("/api/logs")
def get_logs():
...
# =========================================================
# HTML (con mensajes actualizados para reflejar 80/20)
# =========================================================
HTML = r"""<!DOCTYPE html>
<html lang="es">
<head>
...
// mode: "excel" | "excel+paper" | "paper" | "none"
let mode = "none";
...
// Call when an upload finishes (success or error).
function uploadEnd() {
...
async function initNoExcel() {
  ...
  try {
    ...
    // Pre-fill step 4 fields from step 2 inputs
